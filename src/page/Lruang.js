...
export function Lruang() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/pinjamruanglaporan?tanggal_mulai=${startDate}&tanggal_selesai=${endDate}&status=${status}`,
        {
          headers: {
  ...
  const exportToExcel = () => {
    ...
    const worksheet = workbook.addWorksheet("PeminjamanRuangan");
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      link.download = "peminjaman_ruang.xlsx";
      link.click();
    });
  };
              ...
              <div class="inline-block min-w-full py-6 sm:px-6 lg:px-8">
                <div class="overflow-scroll">
                  <table class="min-w-full  text-left text-sm font-light">
                    <thead class="border-b bg-neutral-100 font-medium ">
                      <tr>
