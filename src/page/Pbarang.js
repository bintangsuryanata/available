...
export function Pbarang() {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          "http://127.0.0.1:8000/api/pinjambarang",
          {
            headers: {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(`http://127.0.0.1:8000/api/pinjambarangdelete/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      toast.success('Data Peminjaman Barang Berhasil Dihapus', {
        position: "top-center",
        autoClose: 5000,
  ...
  const handleEdit = (id) => {
    window.location.href = `/epbarang/${id}`;
  };
                              ...
                              <Chip
                                ...
                                className={
                                  status === "disetujui"
                                    ...
                                    : status === "dikembalikan"
                                    ? "w-28 bg-yellow-500 text-xs text-white font-normal"
