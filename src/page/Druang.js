...
const TABLE_HEAD = ["No", "Nama Ruangan", "Status", "Aksi"];

export function Druang() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get("http://127.0.0.1:8000/api/ruang", {
          headers: {
  ...
  const handleView = async (id) => {
    try {
      ...
      window.location.href = `/detailruang/${id}`;
    } catch (error) {
  ...
  const handleEdit = async (id) => {
    try {
      ...
      window.location.href = `/eruang/${id}`;
    } catch (error) {
      console.error("Error fetching edit data:", error);
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(`http://127.0.0.1:8000/api/ruangdelete/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
        ...
        <Card className=" h-full w-full font-poppins p-6 px-32 " shadow={false}>
          ...
          <CardBody className="overflow-scroll mt-4 px-0">
            <table className="w-full min-w-max table-auto ">
              ...
              {isLoading ? (
                ...
                data.map(({ id, nama_ruangan, status }, index) => {
                        ...
                        <div className="gap-3">
                          <div className=" ">
                            <Typography
                              variant="small"
                              color="blue-gray"
                              className="flex items-center font-normal justify-center"
                            >
                              {displayNumber}
                            </Typography>
                          </div>
                      ...
                      <td className={classes}>
                        <div className="flex items-center justify-center">
                          <Chip
                            ...
                            className={
                              status === "tersedia"
                                ? "rounded-2xl bg-green-400 text-white px-8"
                      ...
                      <td className={classes}>
                        <div class="py-3 ">
                          <div class="flex items-center justify-center gap-2">
                            <Button className=" px-1 py-1 bg-blue-600 ">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
          ...
          <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 p-4">
            ...
            <div className="flex gap-2 text-sm">
              ...
              disabled={currentPage === 1}>Sebelumnya</Button>
              <Button variant="outlined"
              onClick={handleNext}
              disabled={currentPage === totalPages}>Selanjutnya</Button>
            </div>
          </CardFooter>
        </Card>
      </div>
